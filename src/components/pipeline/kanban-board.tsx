...
import { DndContext, DragOverlay, closestCorners, useSensor, useSensors, PointerSensor, type DragStartEvent, type DragEndEvent } from "@dnd-kit/core";
...
export function KanbanBoard({ stages: initialStages, customers }: KanbanBoardProps) {
  ...
  // Derived-state pattern (React docs: "Adjusting some state when a prop changes").
  // When router.refresh() delivers fresh server props, the new array reference triggers
  // a render-phase state update — no effect, no extra commit.
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="flex gap-3 overflow-x-auto pb-1">
        {stages.map((stage) => {
          ...
          return (
            <div key={stage.id} className="flex shrink-0 items-center gap-2 rounded-lg border bg-card px-3 py-2">
      ...
      <DndContext sensors={sensors} collisionDetection={closestCorners} onDragStart={handleDragStart} onDragOver={handleDragOver} onDragEnd={handleDragEnd}>
        <div className="flex gap-4 overflow-x-auto pb-4">
          {stages.map((stage) => (
            <KanbanColumn key={stage.id} stage={stage} deals={stage.deals} stages={allStages} customers={customers} isOver={overId === stage.id} />
          ))}
        </div>

        <DragOverlay>{activeDeal ? <DealCardContent deal={activeDeal} isDragging /> : null}</DragOverlay>
      </DndContext>
    </div>
  );
}
